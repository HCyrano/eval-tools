"""
train_FM_global.py  —  FM global pour Roxane (moteur Othello)
=============================================================
Architecture :
...
Usage :
  python train_FM_global.py --epochs 20 --rank 8 --lr 0.02 --preload
  python train_FM_global.py --epochs 20 --resume --preload
  python train_FM_global.py --epochs 3 --stages 30-39    # test streaming
"""
...
# =============================================================================
# Step SGD  (gradient sur w0_stage et V uniquement, w_stage figé)
# =============================================================================

def sgd_step(indices_b, counts_b, scores_b, w_stage, w0_stage, V, lr, l2_V):
    """
    indices_b : (b, K) int32
    ...
    # Gradient w0
    w0_stage -= lr * err.mean()

    # Gradient V (avec comptages)
    ...
    # Gradient clipping — évite les explosions sur indices rares
    ...
    return w0_stage, V, float(np.mean(err ** 2))
...
def parse_args():
    ...
    p.add_argument("--lr_decay",        type=float, default=0.92,
...
def main():
    ...
    if args.resume and CHECKPOINT.exists():
        ...
        lr          = ckpt.get("lr", args.lr * (args.lr_decay ** start_epoch))
        print(f"  Époque {start_epoch}, RMSE={best_rmse:.6f}, "
              f"no_improve={no_improve}, lr={lr:.6f}")
    else:
        print("\nInitialisation...")
        w0_vec      = np.zeros(N_STAGES, dtype=np.float32)
        V           = rng.normal(0, 0.001, (args.rank, N_INDEX)).astype(np.float32)
        start_epoch = 0
        best_rmse   = float("inf")
        no_improve  = 0
        lr          = args.lr
    ...
    # --- Évaluation finale ---
    print(f"\n{'='*60}")
    print("Évaluation finale (5 stages aléatoires)")
    print(f"{'='*60}")
    for stage in sorted(rng.choice(stages, min(5, len(stages)), replace=False)):
        t0 = time()
        indices_mat, counts_mat, scores = charger_stage(stage)
        if indices_mat is None:
            continue
