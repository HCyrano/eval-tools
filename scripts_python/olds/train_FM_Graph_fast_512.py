...
def to_int16(arr, scale):
    """Convertit un tableau float en int16 avec le scale donné et clipping."""
    return np.round(arr * scale).clip(-32768, 32767).astype(np.int16)

def export_fixed_point(w0_vec, V, stages):
    # 1. Export de w0 (toujours au SCALE de base, ex: 256)
    #    Indispensable pour rester aligné avec les poids linéaires w
    w0_ints = {s: int(round(w0_vec[s] * SCALE)) for s in stages}
    with open(FM_W0_TXT, "w") as f:
        for s in range(N_STAGES):
            f.write(f"{w0_ints.get(s, 0)}\n")

    # 2. Export de V avec un scale doublé (ex: 512)
    current_v_scale = 2 * SCALE
    
    # Correction ici : on appelle bien to_int16 avec les deux arguments
    V_aos = to_int16(V, current_v_scale).T  # Transposition pour accès mémoire contigu (AoS)
    V_aos.tofile(FM_V_BIN)

    # 3. Stats et vérification
    rank    = V.shape[0]
    size_mb = (V.size * 2) / 1024 ** 2
    
    # Calcul de l'erreur réelle de quantification
    V_err = np.max(np.abs(V - V_aos.T.astype(np.float32) / current_v_scale))
    
    print(f"  Export Fixed-Point :")
    print(f"    w0 scale: {SCALE}")
    print(f"    V  scale: {current_v_scale}")
    print(f"    Fichier V: {FM_V_BIN} ({size_mb:.1f} Mo)")
