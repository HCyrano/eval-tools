...
def charger_stage(stage):
    ...
    with open(path, "r") as f:
        for line in f:
            ...
            # Tous les entiers sauf le dernier sont des indices de features
            # On les conserve tels quels, doublons inclus (count=1 partout)
...
@njit(cache=True)
def fm_predict(indices_mat, w_stage, w0_stage, V):
    """
    indices_mat : (n, K)      int32   — PAD_IDX = N_INDEX comme sentinelle
    ...
    for i in range(n):
        ...
        for k in range(K):
            idx = indices_mat[i, k]
            if idx == PAD_IDX:
                break              # les PAD sont en fin de ligne
            lin += w_stage[idx]

        # --- Partie FM (count=1 partout) ---
...
@njit(cache=True)
def adam_step(indices_b, scores_b, w_stage, w0_stage, V,
    ...
    # --- Forward + erreur ---
    yhat = fm_predict(indices_b, w_stage, w0_stage, V)
    err  = yhat - scores_b   # (b,)
    ...
    # --- Indices uniques du batch (pour mise a jour sparse) ---
    # On collecte tous les indices non-PAD du batch
    ...
    # --- sum_V[i, r] = Σ_k V[r, indices_b[i,k]]  (count=1) ---
...
def warmup_jit(rank):
    ...
    fm_predict(b_dummy, w_dummy, np.float32(0.0), V_dummy)
    adam_step(b_dummy, s_dummy, w_dummy, np.float32(0.0),
...
def main():
    ...
    if not (args.resume and CHECKPOINT.exists()):
        ...
        for stage in stages:
            ...
            if idx is None:
                continue
            # Somme des poids lineaires pour chaque ligne (count=1)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.preload and val_n > 0:
            ...
            for stage in np.unique(val_stage_ids):
                ...
                yhat_v = fm_predict(idx_v, w_by_stage[stage],
    ...
    print(f"{'='*60}")
    stages_plot, rmse_fm_plot, rmse_lin_plot, rmse_val_plot = [], [], [], []
    ...
    for stage in sorted(stages):
        ...
        yhat     = fm_predict(idx, w_by_stage[stage], np.float32(w0_vec[stage]), V)
        rmse     = float(np.sqrt(np.mean((yhat - sc) ** 2)))
        mae      = float(np.mean(np.abs(yhat - sc)))

        # RMSE lineaire baseline (count=1 : simple somme des poids)
        mask_valid = idx < N_INDEX
        w_ext    = np.where(mask_valid,
                            w_by_stage[stage][np.minimum(idx, N_INDEX - 1)],
                            0.0)
        ...
        if args.preload and val_n > 0:
            ...
            if mask_v.sum() > 0:
                yhat_v = fm_predict(val_indices[mask_v],
                                    w_by_stage[stage],
                                    np.float32(w0_vec[stage]), V)
