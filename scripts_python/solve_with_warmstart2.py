...
if x0_reduit is not None:
    # On résout A·Δx = b - A·x0, puis x = x0 + Δx
    residual_b = scores_train - A_train_reduite @ x0_reduit
    delta_x = lsmr(A_train_reduite, residual_b, atol=1e-4, btol=1e-4, damp=1e-3)[0]
    x_reduit = x0_reduit + delta_x
else:
    x_reduit = lsmr(A_train_reduite, scores_train, atol=1e-4, btol=1e-4, damp=1e-3)[0]
